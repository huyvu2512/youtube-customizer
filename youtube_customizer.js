    (function() {
        ...
        function addCustomStyles() {
            ...
            style.textContent = `
                ...
                để trình duyệt tự báo khi Popup xuất hiện */
            ...
            #movie_player.seeking-mode .ytp-chrome-bottom,
                ...
                /* Ẩn mã quốc gia (VN) cạnh Logo */
                ytd-topbar-logo-renderer #country-code {
                    display: none !important;
                }

                /* GIẢI QUYẾT TRIỆT ĐỂ CẤU TRÚC DOM MỚI CỦA YOUTUBE (Kiến trúc Span Độc Lập) */
                /* Ẩn TOÀN BỘ các thẻ con gốc của YouTube bên trong ytd-logo (bất kể dùng yt-icon, div hay svg) */
                ...
                /* Thẻ container chứa Logo Premium độc lập của chúng ta */
                .custom-premium-logo {
                    display: flex;
                    align-items: center;
                    width: 101px !important;
                    height: 20px !important;
                    color: var(--yt-spec-text-primary, white); /* Tự động đổi màu trắng/đen theo Dark/Light mode */
                    pointer-events: none;
                }
        ...
        // --- 2.1: KHÔI PHỤC LOGO PREMIUM (Siêu nhẹ, Chống đè & Vượt Trusted Types) ---
        // Khởi tạo Trusted Types Policy (Thêm window. để sửa lỗi ESLint no-undef)
        ...
        // Khôi phục logic chèn Logo Premium Độc Lập (Bất tử trước mọi thay đổi DOM của YouTube)
        setInterval(() => {
            let ytdLogos = document.querySelectorAll("ytd-logo");
            ytdLogos.forEach(logo => {
                // Đảm bảo ytd-logo và các thẻ bọc ngoài không bị cắt chữ Premium
                ...
                // Chèn thẻ span độc lập chứa Logo Premium (nếu chưa có)
                ...
                if (!customSpan) {
                    ...
                    customSpan.innerHTML = `<svg viewBox="0 0 101 20" width="101" height="20" preserveAspectRatio="xMinYMid meet">${logoSVG}</svg>`;
                    logo.appendChild(customSpan);
                    logo.setAttribute('is-red-logo', '');
                } else if (!customSpan.innerHTML.includes('M32.1819')) {
                    // Phòng hờ YouTube ngầm ghi đè nội dung của customSpan
                    customSpan.innerHTML = `<svg viewBox="0 0 101 20" width="101" height="20" preserveAspectRatio="xMinYMid meet">${logoSVG}</svg>`;
                }
            });
        }, 500);

        // Xử lý Đóng Cảnh báo Adblock tự động (Dùng CSS Event để bắt chính xác lúc nó bật lên)
