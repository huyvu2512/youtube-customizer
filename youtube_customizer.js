  ...
  function scanAndTagFeedContent(scope) {
    ...
    if (currentConfig.hideMixes) {
      const mixCards = root.querySelectorAll(
        "ytd-rich-item-renderer, ytd-video-renderer, ytd-compact-video-renderer, ytd-radio-renderer, ytd-compact-radio-renderer, ytd-playlist-renderer, ytd-compact-playlist-renderer"
      );
      mixCards.forEach((card) => {
        if (card.classList.contains("ytc-item-mix")) return;
        const tag = card.tagName.toLowerCase();
        if (tag === "ytd-radio-renderer" || tag === "ytd-compact-radio-renderer" || tag === "ytd-playlist-renderer" || tag === "ytd-compact-playlist-renderer") {
