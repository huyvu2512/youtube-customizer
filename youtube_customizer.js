...
(() => {
  ...
  function hasLiveOrChatSupport() {
    ...
    const chatEl = document.querySelector(
      'ytd-live-chat-frame, #chat.ytd-watch-flexy, #chat-container, iframe#chatframe, #chat-teaser, #teaser, [target-id*="chat" i], [target-id="engagement-panel-live-chat"], ytd-engagement-panel-section-list-renderer[target-id*="chat" i], .ytp-live-chat-button, .ytp-chat-button, button[aria-label*="trò chuyện" i], button[aria-label*="chat" i]'
    );
    return !!chatEl;
  }
  ...
  function autoExpandDescriptionIfCollapsed() {
    ...
    const expander = document.querySelector(
      "ytd-watch-metadata ytd-text-inline-expander[is-collapsed], #description ytd-text-inline-expander[is-collapsed], ytd-text-inline-expander#description-inline-expander[is-collapsed]"
    );
    if (expander) {
  ...
  function hideNativeChatElements() {
    ...
    const teasers = document.querySelectorAll(
      '#chat-teaser, #teaser, ytd-live-chat-frame[collapsed], ytd-engagement-panel-section-list-renderer[target-id*="chat" i], [target-id="engagement-panel-live-chat"], ytd-live-chat-frame#chat, #chat.ytd-watch-flexy'
    );
    teasers.forEach((t) => {
    ...
    const openPanelBtns = document.querySelectorAll(
      'ytd-item-section-renderer button[aria-label*="Mở bảng điều khiển" i], ytd-item-section-renderer button[aria-label*="Open panel" i], #related button[aria-label*="Mở bảng điều khiển" i], #below button[aria-label*="Mở bảng điều khiển" i]'
    );
  ...
  function autoCollapseNativeChatIfOpen() {
    ...
    if (!isCollapsed) {
      const hideBtn = chatFrame.querySelector(
        '#show-hide-button button, #close-button button, #close-button, [aria-label*="Ẩn cuộc trò chuyện" i], [aria-label*="Hide chat" i]'
      );
      if (hideBtn) {
  ...
  var userManuallyOpenedChat, hasAutoCollapsedChatForCurrentVideo, hasAutoExpandedDescriptionForCurrentVideo, throttledHideNativeChatElements, chatElementsObserver, chatToggleListenersBound, ensureNativeLiveChatRunning, bgChatIframe, currentBgVideoId, UNICODE_EMOJI_REGEX2;
  var init_chatObserver = __esm({
    "src/chat/chatObserver.js"() {
      ...
      hasAutoCollapsedChatForCurrentVideo = false;
      hasAutoExpandedDescriptionForCurrentVideo = false;
      throttledHideNativeChatElements = rafThrottle(hideNativeChatElements);
      chatElementsObserver = null;
  ...
  __export(chat_exports, {
    ...
    autoExpandDescriptionIfCollapsed: () => autoExpandDescriptionIfCollapsed,
    ...
    hasAutoExpandedDescriptionForCurrentVideo: () => hasAutoExpandedDescriptionForCurrentVideo,
    hideNativeChatElements: () => hideNativeChatElements,
    ...
    setupChatElementsObserver: () => setupChatElementsObserver,
    ...
    throttledHideNativeChatElements: () => throttledHideNativeChatElements,
