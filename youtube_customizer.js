...
(() => {
  ...
  function ensureNativeChatStreaming() {
    ...
    if (chat.hasAttribute("collapsed")) {
      const expandBtn = document.querySelector(
        '#show-hide-button button, ytd-live-chat-frame #show-hide-button button, #chat-container #show-hide-button button, ytd-button-renderer#show-hide-button button, #chat-messages #show-hide-button button, [aria-label*="Hiện cuộc trò chuyện"], [aria-label*="Show chat"], [aria-label*="Live chat"]'
      );
      if (expandBtn) {
        try {
          expandBtn.click();
  ...
  function manageNativeChatVisibility() {
    const chat = document.querySelector("ytd-live-chat-frame#chat, #chat.ytd-watch-flexy, #chat");
    if (!chat) return;
    ...
    if (!panel) {
      ...
      panel.innerHTML = safeHTML(`
            <div class="ytc-header">
                ...
                <span class="ytc-header-badge">v3.0.2</span>
            </div>
