  ...
  function setupChatToggleListeners() {
    ...
    document.addEventListener("click", (e) => {
      ...
      const playerChatBtn = e.target.closest(
        '.ytp-live-chat-button, .ytp-chat-button, button[data-tooltip-target-id="ytp-live-chat-button"], button[data-tooltip-target-id*="chat" i], .ytp-button[aria-label*="trò chuyện" i], .ytp-button[aria-label*="chat" i], .ytp-button[title*="trò chuyện" i], .ytp-button[title*="chat" i]'
      );
      if (playerChatBtn) {
        const player = document.querySelector("#movie_player, .html5-video-player");
      ...
      const isShowBtn = !!e.target.closest(
        '#show-hide-button button, button#show-button, #show-button, [aria-label*="Hiện cuộc trò chuyện" i], [aria-label*="Mở rộng cuộc trò chuyện" i], [aria-label*="Mở bảng điều khiển" i], [aria-label*="Show chat" i], [aria-label*="Expand live chat" i], [aria-label*="Open panel" i], ytd-live-chat-frame[collapsed] #teaser, ytd-live-chat-frame[collapsed] ytd-button-renderer, ytd-live-chat-frame[collapsed] yt-button-shape'
      );
      ...
      const isHideBtn = !!e.target.closest(
        'ytd-live-chat-frame #show-hide-button button, ytd-live-chat-frame #close-button button, ytd-live-chat-frame #close-button, #panels-full-bleed-container #close-button, #panels-full-bleed-container [aria-label*="Đóng" i], #panels-full-bleed-container [aria-label*="Close" i], [aria-label*="Ẩn cuộc trò chuyện" i], [aria-label*="Thu gọn cuộc trò chuyện" i], [aria-label*="Hide chat" i], [aria-label*="Collapse live chat" i], [aria-label*="Close chat" i]'
      );
