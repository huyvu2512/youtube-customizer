// ==UserScript==
// YouTube Customizer v2.0 — https://github.com/huyvu2512/youtube-customizer
// ==/UserScript==
(function() {
    'use strict';
    ...
    function addCustomStyles() {
        ...
        style.textContent = `
            ...
            #cinematics,
            ytd-cinematics-renderer,
            .ytp-ambient-mode-rendering-container {
            ...
            /* Lưới 4 cột cân đối trên màn hình rộng (>= 1024px) */
            ...
            /* Ẩn các banner quảng cáo, khuyến mãi giao diện */
            ...
            ytd-logo yt-icon#logo-icon,
            ytd-logo svg:not(.custom-premium-logo svg) {
                display: none !important;
            }
    ...
    function applyHomeGridFourColumns() {
        ...
        grids.forEach((grid) => {
            if (!grid.classList.contains('ytc-grid-4')) {
                grid.classList.add('ytc-grid-4');
            }
            grid.style.setProperty('--ytd-rich-grid-items-per-row', '4', 'important');
            grid.style.setProperty('--ytd-rich-grid-posts-per-row', '4', 'important');
    ...
    function canUseAsdKeys(player) {
        ...
        // Kiểm tra native :hover hoặc đang fullscreen - 0% lag, không cần mouseover listener toàn cục!
    ...
    function bindGlobalKeys() {
        ...
        document.addEventListener('keydown', (e) => {
            // Tránh can thiệp khi đang gõ IME (Unikey / EVKey) hoặc đang nhập form
            ...
            // --- Điều khiển bằng A / S / D (Dựa theo e.code vật lý, tương thích Unikey) ---
            ...
            // Vô hiệu hóa phím bấm nhầm trên Numpad
    ...
    // 6. KHỞI CHẠY VÀ XỬ LÝ SPA (Single Page Application)
    function onNavigate() {
        applyHomeGridFourColumns();
        updateAllLogos();
        bindGlobalKeys();
    }
    ...
    // Lắng nghe sự kiện chuyển trang nội bộ của YouTube
    document.addEventListener('yt-navigate-finish', onNavigate);

    // Hỗ trợ cập nhật logo nếu YouTube load chậm ban đầu
    let initLogoRetries = 0;
    const logoInterval = setInterval(() => {
        initLogoRetries++;
        updateAllLogos();
        if (initLogoRetries > 8 || document.querySelector('.custom-premium-logo')) {
